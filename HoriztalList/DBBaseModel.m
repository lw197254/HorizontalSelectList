...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    DBHelper *dbHelper = [DBHelper sharedHelper];
    // 如果要支持事务
    [dbHelper.dbQueue inTransaction:^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
        for (DBBaseModel *model in array) {
            //保存修改时间
            NSTimeInterval time = [[NSDate date]timeIntervalSince1970];
            NSString *str = [NSString stringWithFormat:@"%.0f",time];
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                ...
                if ([columeType isEqualToString:SQLTEXT]) {
                    [dict setValue:[resultSet stringForColumn:columeName]  forKey:columeName];
                    [model setValue:[resultSet stringForColumn:columeName] forKey:columeName];
                } else {
                    [model setValue:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:columeName]] forKey:columeName];
                    [dict setValue:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:columeName]] forKey:columeName];
                }
            }
...
//            [users addObject:model];
   ...
   return [NSArray yy_modelArrayWithClass:[self class] json:users];
//    return users;
}
